...
interface HeaderProps {
    currentView: 'Paralegal' | 'Attorney';
    setCurrentView: (view: 'Paralegal' | 'Attorney') => void;
}
        ...
        <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-50">
            <div className="container mx-auto px-6 h-20 flex items-center justify-between">
                ...
                <div className="flex items-center bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-inner">
                    <button
                        onClick={() => setCurrentView('Paralegal')}
                        className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${currentView === 'Paralegal'
                    ...
                    <button
                        onClick={() => setCurrentView('Attorney')}
                        className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${currentView === 'Attorney'
                                ? 'bg-white text-brand-blue shadow-md transform scale-[1.02]'
                                : 'text-slate-500 hover:text-slate-800'
                            }`}
                    >
                ...
                <div className="flex items-center gap-4 pl-4 border-l border-slate-200">
                    <div className="hidden lg:block text-right leading-tight">
                        <p className="text-xs font-black text-slate-800 uppercase tracking-wider">
                            {currentView === 'Paralegal' ? 'Jennifer Park' : 'Robert Chen'}
                        </p>
